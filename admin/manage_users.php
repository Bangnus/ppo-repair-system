...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    ...
    <button onclick="showAddModal()" class="px-6 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-semibold rounded-xl shadow-lg">
        ➕ เพิ่มผู้ใช้
    </button>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                ?>
                    <tr class="hover:bg-gray-100/50 transition-colors <?= $rowColors[$user['role']] ?? '' ?>">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-gradient-to-br <?= $avatarColors[$user['role']] ?? 'from-gray-400 to-gray-600' ?> flex items-center justify-center text-white font-bold">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick='showEditModal(<?= json_encode(["id" => $user["id"], "fullname" => $user["fullname"], "role" => $user["role"]]) ?>)' class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg">✏️</button>
                                <?php if ($user['id'] !== $_SESSION['user_id'] && $user['repair_count'] == 0): ?>
                                    <form method="POST" class="inline" id="deleteForm<?= $user['id'] ?>">
                                        <input type="hidden" name="id" value="<?= $user['id'] ?>">
                                        <input type="hidden" name="delete" value="1">
                                        <button type="button" onclick="confirmDelete(<?= $user['id'] ?>, '<?= e($user['fullname']) ?>')" class="p-2 text-red-500 hover:bg-red-50 rounded-lg">🗑️</button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        </td>
    ...
    <div class="glass rounded-2xl shadow-2xl p-6 w-full max-w-md" onclick="event.stopPropagation()">
        ...
        <form method="POST" class="space-y-4">
            <div>
                ...
                <input type="text" name="username" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">รหัสผ่าน *</label>
                <input type="password" name="password" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">ชื่อ-นามสกุล *</label>
                <input type="text" name="fullname" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">บทบาท *</label>
                <select name="role" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 outline-none">
            ...
            <div class="flex gap-3 pt-4">
                <button name="add" class="flex-1 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-semibold rounded-xl">บันทึก</button>
                <button type="button" onclick="hideAddModal()" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-xl">ยกเลิก</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="glass rounded-2xl shadow-2xl p-6 w-full max-w-md" onclick="event.stopPropagation()">
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="password" name="password" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 outline-none" placeholder="••••••••">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">ชื่อ-นามสกุล *</label>
                <input type="text" name="fullname" id="editFullname" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">บทบาท *</label>
                <select name="role" id="editRole" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 outline-none">
                    <option value="user">👤 User</option>
                    <option value="supervisor">🔰 Supervisor</option>
                    <option value="manager">💼 Manager</option>
                    <option value="admin">👑 Admin</option>
            ...
            <div class="flex gap-3 pt-4">
                <button name="edit" class="flex-1 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold rounded-xl">บันทึก</button>
                <button type="button" onclick="hideEditModal()" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-xl">ยกเลิก</button>
            </div>
        </form>
    </div>
</div>
