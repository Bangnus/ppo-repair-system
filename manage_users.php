...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    if ($stmt->fetchColumn() > 0) {
        ...
        $stmt = $pdo->prepare("INSERT INTO users (username, password, fullname, role, department_id) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$username, $password, $fullname, $role, $deptId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit'])) {
    ...
    if (!empty($_POST['password'])) {
        $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
        $stmt = $pdo->prepare("UPDATE users SET fullname = ?, role = ?, department_id = ?, password = ? WHERE id = ?");
        $stmt->execute([$fullname, $role, $deptId, $password, $id]);
    } else {
        $stmt = $pdo->prepare("UPDATE users SET fullname = ?, role = ?, department_id = ? WHERE id = ?");
        $stmt->execute([$fullname, $role, $deptId, $id]);
    }
...
$users = $pdo->query("
    SELECT u.*, d.name as department_name,
           (SELECT COUNT(*) FROM repairs WHERE user_id = u.id) as repair_count
    FROM users u 
        ...
        <table class="w-full">
            <thead class="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold uppercase">แผนก</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-full bg-gradient-to-br <?= $user['role'] === 'admin' ? 'from-emerald-400 to-emerald-600' : 'from-amber-400 to-amber-600' ?> flex items-center justify-center text-white font-bold">
                        ...
                        <td class="px-6 py-4 text-gray-600">
                            <?= $user['department_name'] ? e($user['department_name']) : '-' ?></td>
                        <td class="px-6 py-4">
                            <span
                                class="px-3 py-1 rounded-full text-xs font-medium <?= $user['role'] === 'admin' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' ?>">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <?php if ($user['id'] !== $_SESSION['user_id'] && $user['repair_count'] == 0): ?>
                                    <form method="POST" class="inline" onsubmit="return confirm('ต้องการลบผู้ใช้นี้?')">
...
<div id="addModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="glass rounded-2xl shadow-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto"
...
<script>
    ...
    function showEditModal(user) {
        ...
        document.getElementById('editDept').value = user.department_id || '';
