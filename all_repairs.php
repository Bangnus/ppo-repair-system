                    ...
                    <?php foreach ($repairs as $repair): ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-4 font-mono text-sm text-gray-500">
                                #<?= str_pad($repair['id'], 4, '0', STR_PAD_LEFT) ?></td>
                            ...
                            <td class="px-4 py-4 max-w-xs">
                                <p class="text-sm text-gray-600 truncate" title="<?= e($repair['problem']) ?>">
                                    <?= e($repair['problem']) ?>
                                </p>
                            </td>
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4 text-xs">
                                ...
                                <?php endif; ?>
                                <?php if ($repair['completed_approvals'] > 0 && $repair['status'] === 'in_progress'): ?>
                                    <span class="text-emerald-600">🟢 เสร็จ: <?= $repair['completed_approvals'] ?>/2</span>
                                <?php endif; ?>
                                <?php if (!$repair['in_progress_approvals'] && !$repair['completed_approvals']): ?>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form method="POST" class="inline" onsubmit="return confirm('ต้องการลบรายการนี้?')">
                                        <input type="hidden" name="repair_id" value="<?= $repair['id'] ?>">
                                        <button name="delete"
                                            class="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs font-medium rounded-lg transition-colors">🗑️</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
...
<div id="imageModal" class="hidden fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4"
    ...
    <div class="relative max-w-4xl max-h-[90vh]" onclick="event.stopPropagation()">
        <button onclick="hideImageModal()"
            class="absolute -top-10 right-0 text-white hover:text-red-400 text-3xl transition-colors">✕</button>
        <img id="modalImage" src="" class="max-w-full max-h-[80vh] rounded-2xl shadow-2xl border-4 border-white">
    </div>
</div>

<!-- Hidden image data for modal -->
<?php foreach ($repairs as $repair): ?>
    <?php if ($repair['image_base64']): ?>
